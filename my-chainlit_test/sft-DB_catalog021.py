# chainlit run sft-DB_catalog021.py -w
...
【テーブル辞書(dd_tables)のデータ定義】

| No  | フィールド名              | 論理名       | 型          | 制約       | 備考                                |
| --- | ------------------- | --------- | ---------- | -------- | --------------------------------- |
| 1   | schema_name         | スキーマ名     | VARCHAR    | NOT NULL | データベース名                           |
| 2   | object_name         | オブジェクト名   | VARCHAR    | NOT NULL | テーブル／ビューの物理名                      |
| 4   | object_type         | オブジェクト種別  | VARCHAR    | NOT NULL | table/viewの区分け                    |
| 3   | business_category   | 業務カテゴリ    | VARCHAR    |          | どの業務領域のデータかを一目で分かるようにする           |
| 5   | business_title      | 業務名称      | VARCHAR    | NOT NULL | テーブル／ビューの論理名                      |
| 6   | business_desc       | 業務定義      | VARCHAR    |          | テーブル／ビュー内容の説明                     |
...
【カラム辞書（dd_columns）のデータ定義】

| No  | フィールド名           | 論理名     | 型               | 制約       | 備考                          |
| --- | ---------------- | ------- | --------------- | -------- | --------------------------- |
| 1   | schema_name      | スキーマ名   | VARCHAR         | NOT NULL | データベース名                     |
| 2   | object_name      | オブジェクト名 | VARCHAR         | NOT NULL | テーブル／ビューの物理名                |
| 3   | column_name      | カラム名    | VARCHAR         | NOT NULL | カラムの物理名                     |
| 4   | ordinal_position | 順番      | NUMERIC (33, 0) |          | テーブル中の順番                    |
| 5   | business_name    | 業務名称    | VARCHAR         | NOT NULL | カラムの論理名                     |
| 6   | data_type        | データ型    | VARCHAR         | NOT NULL | データ型                        |
| 7   | business_desc    | 業務定義    | VARCHAR         |          | 業務の内容説明                     |
...
- 「業務の探索における業務」とは、
  dd_tables / dd_columns に登録されている
  以下の文字列情報に、指定語が **文字列として含まれる**場合を指す。
  対象項目：
  - テーブル辞書(dd_tables) の以下項目： 
    - オブジェクト名(object_name)
    - 業務名称(business_title)
    - 業務定義(business_desc)
  - カラム辞書(dd_columns)の以下項目：
    - カラム名(column_name)
    - 業務名称(buusiness_name)
    - 業務定義(business_desc)

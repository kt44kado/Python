"""
DBカタログ(DrSumの辞書テーブル)自然言語チャットクライアント　DB-Catalog_chat01.py
...
──────────────────────────────
【手順1：関連カラムの全件抽出】
・指定語で `M_COLUMN_DICTIONARY` の column_physical_name,column_logical_name,column_desc を LIKE 検索します。

【手順2：関連テーブルの全件抽出】
・指定語で `M_OBJECT_DICTIONARY` のobject_physical_name,object_logical_name,object_desc を LIKE 検索します。

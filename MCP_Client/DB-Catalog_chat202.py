...
DBカタログ(DrSumの辞書テーブル)自然言語チャットクライアント
    ...
    DB-Catalog_chat201.py ツールの実行結果から改行やインデントを削除してトークン節約するバージョン
    DB-Catalog_chat202.py ツールの実行結果を圧縮してトークン節約するバージョン
...
# トークン節約のため、ツールの実行結果を圧縮する関数
def compact_tool_result(tool_name, result):
    ...
    if content and isinstance(content, list):
        ...
        try:
            ...
            print("RECORDS COUNT:", len(actual.get("records", [])))  # ← rowsからrecordsに変更
            print("RECORDS PREVIEW:", actual.get("records", [])[:2]) # ← 中身も確認
        except json.JSONDecodeError:
            print("JSON PARSE FAILED")
            return result  # パース失敗時はそのまま返す

    if tool_name == "execute_select":
        rows = actual.get("records", actual.get("rows", actual.get("data", [])))
        actual["records"] = rows[:20]
        actual["truncated"] = len(rows) > 20
        # columnsなど他のキーはそのまま保持される

    if tool_name == "get_table_list":
        tables = actual.get("tables", [])
        actual["tables"] = [{"name": t.get("name"), "type": t.get("type")} for t in tables[:50]]
        actual["total"] = len(tables)
        actual["truncated"] = len(tables) > 50

    return actual  # フラットに返す
...
def execute_tool(tool_name: str, arguments: dict) -> str:
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            print("INNER KEYS:", inner.keys() if isinstance(inner, dict) else "NOT DICT")
            print("INNER PREVIEW:", str(inner)[:300])

        result = compact_tool_result(tool_name, result)
        json_str = json.dumps(result, ensure_ascii=False, separators=(",", ":"))
        print("FINAL JSON:", json_str[:500])  # ← 追加
        return json_str

    
        # return json.dumps(result, ensure_ascii=False, indent=2)
        # return json.dumps(result, ensure_ascii=False, separators=(",", ":")) # トークン節約のため、インデントなし＆スペースなしで出力

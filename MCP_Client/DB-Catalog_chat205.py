...
DBカタログ(DrSumの辞書テーブル)自然言語チャットクライアント
    ...
    DB-Catalog_chat205.py 会話履歴を削除するバージョン（トークン節約のため、ツール呼び出し後の会話履歴にツールの実行結果を追加しない）
...
MAX_TURNS = 6  # user/assistant を何往復残すか

def trim_messages(messages):
    # systemは残す
    system = messages[:1]
    rest = messages[1:]
    # toolメッセージも含めると増えやすいので、ここでは後述の「tool結果の要約」も併用推奨

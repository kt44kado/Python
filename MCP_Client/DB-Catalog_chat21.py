...
DBカタログ(DrSumの辞書テーブル)自然言語チャットクライアント
    ...
    DB-Catalog_chat21.py トークンの削減作業
...
def execute_tool(tool_name: str, arguments: dict) -> str:
    ...
    try:
        ...
        # ---- DEBUG: execute_selectの戻り構造を確認（キーだけ表示） ----
        if tool_name == "execute_select" and isinstance(result, dict):
            print("🔎 execute_select result top keys:", list(result.keys()))
            # よくあるキーの有無をチェック
            for k in ["rows", "data", "columns", "result", "success", "error", "message"]:
                if k in result:
                    v = result[k]
                    t = type(v).__name__
                    # 件数/サイズだけ
                    if isinstance(v, list):
                        ...
                        print(f"  - {k}: dict keys={list(v.keys())[:20]}{'...' if len(v.keys())>20 else ''}")
                    else:
                        s = str(v)
                        print(f"  - {k}: {t} {s[:120]}{'...' if len(s)>120 else ''}")
        # ---------------------------------------------------------
        # ---- DEBUG: execute_select の content 構造を確認（キー/型/件数だけ） ----
        if tool_name == "execute_select" and isinstance(result, dict):
            ...
            if isinstance(content, dict):
                ...
                # よくある候補キーの存在とサイズだけ
                for k in ["rows", "data", "records", "columns", "fields", "header", "resultSet"]:
                    if k in content:
                        v = content[k]
                        if isinstance(v, list):
                            print(f"    * {k}: list len={len(v)}")
                            if len(v) > 0:
                                print(f"      - {k}[0] type: {type(v[0]).__name__}")
                        elif isinstance(v, dict):
                            print(f"    * {k}: dict keys={list(v.keys())[:30]}")
                        else:
                            s = "" if v is None else str(v)
                            print(f"    * {k}: {type(v).__name__} head={s[:120]}")

            elif isinstance(content, list):
                print("  - content type: list len=", len(content))
                if content:
                    print("  - content[0] type:", type(content[0]).__name__)

            else:
                print("  - content type:", type(content).__name__)
                s = "" if content is None else str(content)
                print("  - content head:", s[:200], "..." if len(s) > 200 else "")
        
# -------------------------------------------------------------------
        # content が list だった場合、先頭dictのキーを表示
        if isinstance(content, list) and content and isinstance(content[0], dict):
            d0 = content[0]
            print("  - content[0] keys:", list(d0.keys()))
            # よくある候補キーのサイズだけ出す（中身は出さない）
            for k in ["rows", "data", "records", "columns", "fields", "header"]:
                if k in d0:
                    v = d0[k]
                    if isinstance(v, list):
                        print(f"    * content[0].{k}: list len={len(v)}")
                        if v:
                            print(f"      - {k}[0] type: {type(v[0]).__name__}")
                    elif isinstance(v, dict):
                        print(f"    * content[0].{k}: dict keys={list(v.keys())[:30]}")
                    else:
                        s = "" if v is None else str(v)
                        print(f"    * content[0].{k}: {type(v).__name__} head={s[:120]}")
